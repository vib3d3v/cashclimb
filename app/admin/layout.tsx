...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  // Remove the cookie check here — middleware handles it
  return (
    <div className="min-h-screen flex bg-bg">
      ...
      <aside className="w-56 flex-shrink-0 bg-bg-2 border-r border-border flex flex-col">
        <Link href="/" className="flex items-center gap-2 px-5 py-5 border-b border-border">
          <div className="w-7 h-7 bg-gold flex items-center justify-center text-bg font-black text-xs flex-shrink-0"
        ...
        <nav className="flex-1 py-4 px-3 space-y-1">
          {NAV.map(n => (
            <Link
              key={n.href}
              href={n.href}
              className="block px-3 py-2.5 rounded-lg text-sm font-medium text-[#9A9490]
                         hover:text-[#F0EDE8] hover:bg-bg-3 transition-colors"
            >
              {n.label}
            </Link>
          ))}
