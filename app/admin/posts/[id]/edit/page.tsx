...
export default async function EditPostPage({ params }: { params: { id: string } }) {
  ...
  const { data: latestCheck } = await supabase.from('quality_checks').select('*').eq('post_id', params.id).order('created_at', { ascending: false }).limit(1).maybeSingle()
  const evaluation = latestCheck ? { score: latestCheck.score, passed: latestCheck.passed, risk_level: latestCheck.risk_level, checks: latestCheck.checks } : evaluateFinanceArticle({ title: post.title, excerpt: post.excerpt, body: post.body, primaryKeyword: post.primary_keyword, category: post.category, seoTitle: post.seo_title, seoDescription: post.seo_description, coverUrl: post.cover_url })
  ...
  async function updatePost(formData: FormData) {
    ...
    const { error } = await supabase.from('posts').update({ ...payload, quality_score: evaluation.score, risk_level: evaluation.risk_level }).eq('id', params.id)
    if (error) throw error
    await supabase.from('quality_checks').insert({ post_id: params.id, score: evaluation.score, passed: evaluation.passed, risk_level: evaluation.risk_level, checks: evaluation.checks })
  ...
  return (
    <div className="space-y-6">
      <PostSaveToast />
      <div><p className="text-xs font-bold uppercase tracking-widest text-gold">Admin</p><h1 className="mt-2 font-serif text-4xl font-black text-[#F0EDE8]">Edit post</h1></div>
