...
export default function EditPostPage() {
  ...
  const [form, setForm]     = useState({ title: '', excerpt: '', category: 'Investing', author: '', published: false })
  ...
  useEffect(() => {
    fetch(`/api/posts/${id}`, { headers: { 'x-admin-key': adminKey } })
      .then(r => r.json())
      .then(data => {
        setForm({ title: data.title, excerpt: data.excerpt, category: data.category, author: data.author, published: data.published })
  ...
  async function handleImageUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setCoverUrl(data.url); setCoverPreview(URL.createObjectURL(file))
  ...
  if (loading) return <div className="text-[#6A6460] text-sm">Loading post…</div>
    ...
    <div>
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="flex gap-3">
          <button onClick={() => handleSave(false)} className="cc-btn-ghost" disabled={saving}>Save Draft</button>
          <button onClick={() => handleSave(true)}  className="cc-btn-primary" disabled={saving}>
      ...
      <div className="space-y-6">
        <div>
          ...
          <input className="cc-input text-lg" value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} />
        </div>
        <div>
          <label className="cc-label">Excerpt *</label>
          <textarea className="cc-input min-h-[90px] resize-y" value={form.excerpt} onChange={e => setForm(f => ({ ...f, excerpt: e.target.value }))} />
        </div>
        <div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input className="cc-input" value={form.author} onChange={e => setForm(f => ({ ...f, author: e.target.value }))} />
          </div>
        </div>
        <div>
