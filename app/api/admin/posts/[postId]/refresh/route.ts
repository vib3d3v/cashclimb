...
import { refreshExistingPost } from '@/lib/ai-editor'
...
export async function POST(
  ...
  const rewritten = await refreshExistingPost(post)
  ...
  if (updateError || checkError) {
    return NextResponse.json({ error: updateError?.message || checkError?.message || 'Failed to refresh post' }, { status: 500 })
  }
